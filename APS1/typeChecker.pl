...
dec(Context, var(ident(Id), Type), [(Id,Type) | Context]).
...
stat(Context, set(ident(Id), Expr), void) :-expr(Context, ident(Id), T), expr(Context, Expr, T).
...
stat(Context, call(ident(Id), Exprs), void) :-  listTypes(Context, Exprs, Types), inContext(Context, Id, composedType(Types, void)).
